...
from logger.grad_logger import GradientStatsLogger


TEXT_SOMETHING = "Andereson_Mixing_new"
...
def set_flat_params(parameters, flat_vector):
    offset = 0
    for p in parameters:
        numel = p.numel()
...
def MUNBa(
    ...
    grad_logger = GradientStatsLogger()
    ...
    #############################
    p=2                 # Anderson memory
    warmup=100           # steps before enabling AA
    theta_hist = []
    G_hist = []
    residual_hist = []
    ###############################
    for epoch in range(epochs):
        ...
        with tqdm(total=len(forget_dl)) as time_1:
            ...
            for i, (forget_images, forget_labels) in enumerate(forget_dl):
                ...
                # ---- Compute current flat theta ----
                theta_t = get_flat_params(parameters).detach()
                ...
                grads_flat = torch.cat([
                    p.grad.view(-1) for p in parameters
                ]).detach()

                # ---- Define fixed-point map G(theta) = theta - lr * grad ----
                G_theta_t = theta_t - lr * grads_flat

                # ---- Residual f_t = G(theta_t) - theta_t ----
                f_t = (G_theta_t - theta_t).detach().cpu()

                # ---- Store history ----
                theta_hist.append(theta_t.detach().cpu())
                G_hist.append(G_theta_t.detach().cpu())
                residual_hist.append(f_t)
                ...
                # ---- Classical Anderson ----
                if step > warmup and len(residual_hist) == p:
                    lambda_reg = 0.3   # start with 0.3

                    F = torch.stack(residual_hist, dim=1)      # [dim, p]
                    G_stack = torch.stack(G_hist, dim=1)       # [dim, p]
                    G_curr = G_stack[:, -1]                    # current fixed-point step

                    try:
                        # Compute matrices
                        FtF = F.T @ F                          # [p, p]
                        GtG = G_stack.T @ G_stack              # [p, p]
                        GtG_curr = G_stack.T @ G_curr          # [p]

                        # Regularized system
                        A = FtF + lambda_reg * GtG + 1e-6 * torch.eye(p, device=F.device)
                        b = lambda_reg * GtG_curr

                        beta_vec = torch.linalg.solve(A, b)

                        # enforce sum-to-one
                        beta_vec = beta_vec / beta_vec.sum()

                        # build update
                        theta_next = G_stack @ beta_vec

                        # ---- SAFETY: prevent destructive direction ----
                        if torch.dot(theta_next - theta_t.cpu(), G_curr - theta_t.cpu()) < 0:
                            theta_next = G_curr

                        theta_next = theta_next.to(device)
                        set_flat_params(parameters, theta_next)

                    except:
                        set_flat_params(parameters, G_theta_t.to(device))

                else:
                    # Before warmup just use plain fixed-point update
                    set_flat_params(parameters, G_theta_t.to(device))
                ...
                gc.collect()
                if step % 100 == 0:
                    grad_logger.save_npy(f"models/{name}/grads.npy")
                    grad_logger.save_csv(f"models/{name}/grads.csv")
                    grad_logger.print_variances()
