...
def MUNBa(classes,
    ...
    if mask_path:
        ...
        name = f"compvis-nsfw-MUNBa-method_{train_method}-lr_{lr}_E{epochs}_U{num_forget}"
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(forget_dl)) as time_1:
            ...
            for i, batch in enumerate(forget_dl):
                ...
                if (step+1) % 10 == 0:
                    if args.munba:
                        logger.info(f"step: {step}, loss: {loss:.4f}, loss_r: {loss_r * prvs_alpha[0]:.4f}, loss_u: {loss_u * prvs_alpha[1]:.4f}")
                    else:
