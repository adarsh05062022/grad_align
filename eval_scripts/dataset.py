...
def setup_fid_data(class_to_forget, path, image_size, interpolation="bicubic"):
    ...
    #     "imagenette_without_label_0", class_to_forget, transform=transform
    # )
    ...
    for data in train_set:
        if data[1] != class_to_forget:
            real_set.append(data[0])
            cnt += 1
        if cnt >= 1000:
            break
    # real_set = [data[0] for data in train_set if data[1] != class_to_forget]
...
def setup_fid_data_i2p(real_path, path, image_size, interpolation="bicubic"):
    interpolation = INTERPOLATIONS[interpolation]
    transform = get_transform(interpolation, image_size)
