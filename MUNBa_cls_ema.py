...
TEXT_SOMETHING = "with_ema_only_applied_norm_ema_beta_0-70"
        ...
        with tqdm(total=len(forget_dl)) as time_1:
            ...
            for i, (images, labels) in enumerate(forget_dl):
                ...
                if args.munba:
                    ...
                    '''
                    '''
                    # matched with the paper angle code 

                    # Assuming grads[0] and grads[1] are normalized or you handle the magnitudes separately
                    gr = grads[0]
                    gf = grads[1]
                    xi = 1e-8 # Your stabilization constant
                    norm_gr = torch.norm(gr) + xi
                    norm_gf = torch.norm(gf) + xi

                    # Calculate cosine(phi)
                    cos_phi = torch.dot(gr, gf) / (norm_gr * norm_gf + 1e-8)
                    cos_phi = torch.clamp(cos_phi, -1.0 + 1e-6, 1.0 - 1e-6)
                    sin_sq_phi = 1.0 - cos_phi**2
                    

                    # Calculate alpha_r and alpha_f using the exact formulas from the image
                    prvs_alpha[0] = (1.0 / norm_gr) * torch.sqrt( (1.0 - cos_phi) / (sin_sq_phi + xi) )
                    # The second formula for alpha_f can be simplified mathematically, 
                    # but a direct implementation is:
                    prvs_alpha[1] = torch.sqrt( (sin_sq_phi * (1.0 - cos_phi)) / norm_gf**2 )  
                    ...
                    ema_nr  = beta_angle * ema_nr  + (1-beta_angle) * norm_gr
                    ema_nf  = beta_angle * ema_nf  + (1-beta_angle) * norm_gf 

                    norm_gr  = ema_nr  / (1 - beta_angle**angle_step)
                    norm_gf  = ema_nf  / (1 - beta_angle**angle_step)


                    
                    sin_sq_phi = 1.0 - cos_phi**2

                    alpha_0  = (1.0 / norm_gr) * torch.sqrt(
                        (1.0 - cos_phi) / (sin_sq_phi + eps)
                    )
                    ...
                    logger.info(f"BETA_ANGLE: - {beta_angle}")
