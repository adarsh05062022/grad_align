...
TEXT_SOMETHING = "layer_importance_masking_20_precent_forget_50_percent_retain_per_epoch_mask_fisher"
...
def MUNBa(
    ...
    for epoch in range(epochs):
        ...
        beta,
        device,
        percent_forget=0.2,  # top 5% of gradients for forget importance
        percent_retain=0.5,  # bottom 40% of gradients for retain importance (low importance allowed to be forgotten
        )
        logger.info(f"Dual importance masks computed for all layers. Starting training with MUNBa...")
        logger.info(f"Masking method: {TEXT_SOMETHING} | Percent forget: 20% | Percent retain: 50%")
