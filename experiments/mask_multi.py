...
def _accumulate_fisher(
    ...
    for batch_idx,(forget_images, forget_labels) in enumerate(tqdm(forget_dl, desc="[Mask] Accumulating Fisher")):
...
def compute_dual_importance_mask(
    ...
    """
    forget_str = str(sorted(class_to_forget)) if isinstance(class_to_forget, (list, tuple)) else str(class_to_forget)
    if logger:
        logger.info(
            f"[Mask] Computing — classes={forget_str}  density={target_density}  λ={lambda_tradeoff}  "
