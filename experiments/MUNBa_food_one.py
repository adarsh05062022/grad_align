...
from train_scripts.dataset_food import (
    setup_model,
    setup_forget_data,
    setup_remain_data,
)
...
def MUNBa(
    ...
    name = f"compvis-cls_{class_to_forget}-MUNBa-method_{train_method}-lr_{lr}_E{epochs}_U{num_forget}_FOOD101_{TEXT_SOMETHING}"
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(forget_dl)) as time_1:
            ...
            for i, (forget_images, forget_labels) in enumerate(forget_dl):   
                ...
                remain_prompts = [descriptions[int(label)] for label in remain_labels]
                forget_prompts = [descriptions[int(label)] for label in forget_labels]
                #   chnage this 
                pseudo_class = (int(class_to_forget) + 1) % len(FOOD10_CLASSES)
                pseudo_prompts = [descriptions[pseudo_class] for _ in forget_labels]
