...
TEXT_SOMETHING = "layer_importance_masking_5_percentage_per_epoch_mask_fisher"
...
def compute_dual_importance_mask(
    ...
    for g_f, g_r in zip(accum_fisher_f, accum_fisher_r):
        ...
        norm_f = g_f / (g_f.mean() + 1e-8)
        norm_r = g_r / (g_r.mean() + 1e-8)

        # Fisher score
        score = norm_f - lambda_tradeoff * norm_r
        flat_score = score.view(-1)

        k = int(target_density * flat_score.numel())

        if k == 0:
            masks.append(torch.zeros_like(score, dtype=torch.bool))
            continue

        thresh = torch.topk(flat_score, k).values[-1]
        final_mask = score >= thresh
...
def MUNBa(
    ...
    for epoch in range(epochs):
        ...
        beta,
        device,
        target_density=0.05,  # target 5% parameters to update per layer for now (can be tuned)
        lambda_tradeoff=1.0,  #tradeoff between forgetting and retaining, can be tuned
        )
            ...
            for i, (forget_images, forget_labels) in enumerate(forget_dl):   
                ...
                if args.munba:
                    ...
                    for layer_index, (gr, gf, p) in enumerate(zip(grads_r, grads_f, parameters)):
                        ...
                        gr_masked = gr[layer_mask]
                        gf_masked = gf[layer_mask]
                        ...
                        # Build sparse update tensor
                        g_tilde = torch.zeros_like(gr)
                        g_tilde[layer_mask] = alpha_r_l * gr[layer_mask] + alpha_f_l * gf[layer_mask]
